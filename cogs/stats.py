            ...
            async with ClientSession() as session:
                async with session.get("https://api.coingecko.com/api/v3/coins/helix", headers=headers) as response:
                    ...
                    for item in priceData:
                        ...
                        embed.add_field(name="Price (USD)", value="${}".format(item[tickers][0][converted_last]['usd']))
                        embed.add_field(name="Price (BTC)", value="{} BTC".format(item[tickers][0][converted_last]['btc']))
                        embed.add_field(name='\u200b',value='\u200b')
                        embed.add_field(name="Volume (USD)", value="${}".format(item[tickers][0]['volume']))
                        embed.add_field(name="Market Cap", value="${}".format(item[market_data][market_cap]['usd']))
                        embed.add_field(name='\u200b',value='\u200b')
                        embed.add_field(name="% 24h", value="{}%".format(item[market_data]['price_change_percentage_24h']))
                        embed.add_field(name="% 7d", value="{}%".format(item[market_data]['price_change_percentage_7d']))
                        embed.add_field(name="Circulating Supply", value="{} HLIX".format(item[market_data]['circulating_supply']))
                        embed.add_field(name="Total Supply", value="{} HLIX".format(item[market_data]['total_supply']))
